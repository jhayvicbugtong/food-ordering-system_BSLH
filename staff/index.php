...
$attendance_stmt = $conn->prepare("
    ...
    LIMIT 1
");
$attendance_stmt->bind_param("is", $staff_id, $today);
$attendance_stmt->execute();
$today_attendance = $attendance_stmt->get_result()->fetch_assoc();

$latest_payroll_stmt = $conn->prepare("
    SELECT * 
    FROM staff_payroll 
    WHERE staff_id = ?
    ...
    LIMIT 1
");
$latest_payroll_stmt->bind_param("i", $staff_id);
$latest_payroll_stmt->execute();
$latest_payroll = $latest_payroll_stmt->get_result()->fetch_assoc();
?>
...
<style>
  ...
  .source-pill.pickup { background: #ecfdf3; color: #166534; }
  .source-pill.delivery { background: #eff6ff; color: #1d4ed8; }
  .source-pill.pos { background: #fef3c7; color: #92400e; }
  ...
  <main class="main-content">
    ...
    <div class="row g-3 mb-4">
      <div class="col-12 col-md-6 col-xl-3">
        <div class="attendance-card">
          ...
          <div class="small-hint">Your clock-in time today</div>
        </div>
      </div>

      <div class="col-12 col-md-6 col-xl-3">
        <div class="attendance-card">
          <h5>Today Time Out</h5>
          <div class="big-value">
            <?= !empty($today_attendance['time_out']) ? date('h:i A', strtotime($today_attendance['time_out'])) : '--:--' ?>
          </div>
          <div class="small-hint">Your clock-out time today</div>
        </div>
      </div>
      ...
      <div class="col-12 col-md-6 col-xl-3">
        <div class="attendance-card">
          ...
          <div class="small-hint">
            Status: <?= htmlspecialchars($today_attendance['status'] ?? 'No record') ?>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-md-6 col-xl-3">
        <div class="attendance-card">
          <h5>Latest Payroll</h5>
          <div class="big-value">
            ₱<?= number_format((float)($latest_payroll['net_pay'] ?? 0), 2) ?>
          </div>
          <div class="small-hint">
            <?= !empty($latest_payroll) ? htmlspecialchars($latest_payroll['status']) : 'No payroll yet' ?>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="content-card mb-4">
      <div class="content-card-header">
        <div>
          <h2>Attendance Quick Actions</h2>
          <p>Clock in, clock out, and view your attendance or payroll records.</p>
        </div>
      </div>

      <div class="d-flex gap-2 flex-wrap">
        <button type="button" class="btn btn-success quick-btn" onclick="clockIn()">
          <i class="bi bi-box-arrow-in-right me-1"></i>
          Clock In
        </button>

        <button type="button" class="btn btn-dark quick-btn" onclick="clockOut()">
          <i class="bi bi-box-arrow-right me-1"></i>
          Clock Out
        </button>

        <a href="attendance.php" class="btn btn-outline-success quick-btn">
          <i class="bi bi-calendar-check me-1"></i>
          View Attendance
        </a>

        <a href="payroll.php" class="btn btn-outline-dark quick-btn">
            ...
            <?php if ($active_orders && $active_orders->num_rows > 0): ?>
              <?php while ($order = $active_orders->fetch_assoc()): ?>
                ...
                <tr>
                  ...
                  <td>
                    <span style="font-weight:600; white-space:nowrap;">₱<?= number_format($total, 2) ?></span>
                  </td>
...
<script>
  function clockIn() {
    fetch('actions/clock_in.php', { method: 'POST' })
      ...
      .catch(() => {
        Swal.fire('Error', 'Unable to clock in.', 'error');
      });
  }
  ...
  function clockOut() {
    fetch('actions/clock_out.php', { method: 'POST' })
      ...
      .catch(() => {
        Swal.fire('Error', 'Unable to clock out.', 'error');
      });
  }

  function fetchDashboardUpdates() {
    if (document.activeElement && document.activeElement.classList.contains('form-check-input')) return;
