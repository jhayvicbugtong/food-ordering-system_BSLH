<!DOCTYPE html>
<html class="cms4-page" lang="en">
 <head>
  <meta content="text/html; charset=utf-8" http-equiv="content-type"/>
  <meta content="
                    index
                    ,follow
        " name="robots"/>
  <title>
   Strona główna
  </title>
  <link class="cms-favicon-image" href="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/favicon.png?v=0" rel="shortcut icon"/>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&amp;display=swap" rel="stylesheet"/>
  <meta content="
            Welcome in our unique place, which attracts with its original interior but above all the quality of the dishes served. Carefully prepared dishes are characterized by freshness and a lot of ingredients.
" name="description"/>
  <!-- Open Graph Data -->
  <meta content="
                Strona główna
    " property="og:title"/>
  <meta content="https://avocado-en.orderwebsite.com/" property="og:url"/>
  <meta content="restaurant" property="og:type"/>
  <meta content="
                Welcome in our unique place, which attracts with its original interior but above all the quality of the dishes served. Carefully prepared dishes are characterized by freshness and a lot of ingredients.
    " property="og:description"/>
  <meta content="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/cover.jpg" property="og:image"/>
  <!-- // Open Graph Data -->
  <!-- Twitter Card -->
  <meta content="summary" name="twitter:card"/>
  <meta content="
                Strona główna
    " name="twitter:title"/>
  <meta content="
                Welcome in our unique place, which attracts with its original interior but above all the quality of the dishes served. Carefully prepared dishes are characterized by freshness and a lot of ingredients.
    " name="twitter:description"/>
  <meta content="https://avocado-en.orderwebsite.com/" name="twitter:url"/>
  <meta content="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/cover.jpg" name="twitter:image"/>
  <!-- // Twitter Card -->
  <link href="index.html" rel="canonical"/>
  <meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0" name="viewport"/>
  <meta content="IE=edge" http-equiv="X-UA-Compatible">
   <!-- GOOGLE ANALYTICS -->
   <!-- GOOGLE ANALYTICS -->
   <!-- FACEBOOK PIXEL CODE -->
   <!-- FACEBOOK PIXEL CODE -->
   <!-- upmenu marketing automation tracking code -->
   <!-- fonts mockup function - is here only for backward compatibilty. method moved to bottomResources -->
   <!-- CSS LIBS -->
   <link href="https://static.cdn-upm.com/media/1202/topAll.css" rel="stylesheet" type="text/css">
    <!-- CSS LIBS -->
    <!-- EDIT MODE CSS -->
    <!-- EDIT MODE CSS -->
    <!-- LESS STYLES -->
    <link href="https://avocado-en.orderwebsite.com/dynamict/themes/less/v4/98dfb947-4a04-11ed-8bca-525400080621/styles-6/theme-styles.json" rel="stylesheet" type="text/css">
     <!-- // LESS STYLES -->
     <!-- JSON LD Schema -->
     <!-- / JSON LD Schema -->
    </link>
   </link>
  </meta>
 </head>
 <body class="upmenu theme-body page-with-nav">
  <div class="page-navigation">
   <div class="theme-header pos-rlt">
    <div class="container theme-header-inner main-navigation">
     <div class="row">
      <div class="col-sm-12 row-flex flex-middle nav-container nav-preset-1">
       <a class="brand logo-custom m-r clearfix" href="index.html">
        <img alt="Avocado" class="img-responsive inline" height="45" id="logo" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/logo.png?v=0" width="200"/>
       </a>
       <ul class="nav navbar-nav theme-navbar-nav theme-navigation m-l-auto m-r-auto clearfix hidden-xs hidden-sm responsivetab">
        <li class="" role="presentation">
         <a class="active" href="index.html">
          <span class="inline pos-rlt">
           Home
          </span>
          <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
          </i>
         </a>
        </li>
        <li role="presentation">
         <a class="" href="https://avocado-en.orderwebsite.com/about-us">
          <span class="pos-rlt inline">
           About us
          </span>
          <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
          </i>
         </a>
        </li>
        <li role="presentation">
         <a class="" href="https://avocado-en.orderwebsite.com/gallery">
          <span class="pos-rlt inline">
           Gallery
          </span>
          <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
          </i>
         </a>
        </li>
        <li role="presentation">
         <a class="" href="https://avocado-en.orderwebsite.com/contact">
          <span class="pos-rlt inline">
           Contact
          </span>
          <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
          </i>
         </a>
        </li>
       </ul>
       <ul class="nav navbar-nav theme-navbar-nav theme-account m-l hidden-xs hidden-sm">
        <li class="theme-highlight-btn">
         <a class="btn btn-upmenu" href="https://avocado-en.orderwebsite.com/menu">
          Order online
         </a>
        </li>
        <li class="theme-nav-customer-account">
         <a class="dropdown-toggle btn" data-toggle="dropdown" href="index.html#">
          <i class="feather-icon icon-user">
          </i>
         </a>
         <ul class="dropdown-menu dropdown-menu-right">
          <li>
           <a href="https://avocado-en.orderwebsite.com/?login" id="_start-login">
            Sign in
           </a>
          </li>
          <li>
           <a href="https://avocado-en.orderwebsite.com/?registration" id="_start-registration">
            Register
           </a>
          </li>
         </ul>
        </li>
       </ul>
       <div class="m-l-auto visible-xs visible-sm">
        <div class="short-nav-toggle">
         <a class="_toggle-aside-nav" href="index.html#">
          <svg class="v-middle m-t-n-xxs" fill="none" height="20" stroke="currentColor" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
           <path d="M2.5 15H17.5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
           </path>
           <path d="M2.5 10H17.5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
           </path>
           <path d="M2.5 5H17.5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
           </path>
          </svg>
         </a>
        </div>
       </div>
      </div>
     </div>
    </div>
    <aside class="aside-navigation">
     <div class="nav-container slim-scrollbar">
      <a class="brand logo-custom inline clearfix" href="index.html">
       <img alt="Avocado" class="img-responsive" height="45" id="logo" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/logo.png?v=0" width="200"/>
      </a>
      <a class="short-nav-close pull-right _toggle-aside-nav" href="index.html#">
       <svg class="v-middle" fill="none" height="20" stroke="currentColor" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
        <path d="M15 5L5 15" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
        </path>
        <path d="M5 5L15 15" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
        </path>
       </svg>
      </a>
      <div class="row m-t-xl">
       <div class="col-xs-6">
       </div>
       <div class="col-xs-6">
        <ul class="nav theme-navbar-nav theme-nav-customer-account-mobile m-r-auto">
         <div class="theme-nav-customer-account-mobile text-right">
          <a class="dropdown-toggle" data-toggle="dropdown" href="index.html#">
           <i class="feather-icon icon-user">
           </i>
          </a>
          <ul class="dropdown-menu dropdown-menu-right" role="menu">
           <li>
            <a href="https://avocado-en.orderwebsite.com/?login" id="_start-login">
             <span class="inline pos-rlt">
              Sign in
             </span>
            </a>
           </li>
           <li>
            <a href="https://avocado-en.orderwebsite.com/?registration" id="_start-registration">
             <span class="inline pos-rlt">
              Register
             </span>
            </a>
           </li>
          </ul>
         </div>
        </ul>
       </div>
      </div>
      <div class="separator m-t m-b">
      </div>
      <ul class="nav theme-navbar-nav m-r-auto">
       <li class="theme-highlight-btn">
        <a class="btn btn-upmenu" href="https://avocado-en.orderwebsite.com/menu">
         Order online
        </a>
       </li>
      </ul>
      <ul class="nav theme-navbar-nav m-l-auto clearfix">
       <li class="" role="presentation">
        <a class="active" href="index.html">
         <span class="inline pos-rlt">
          Home
         </span>
         <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
         </i>
        </a>
       </li>
       <li role="presentation">
        <a class="" href="https://avocado-en.orderwebsite.com/about-us">
         <span class="pos-rlt inline">
          About us
         </span>
         <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
         </i>
        </a>
       </li>
       <li role="presentation">
        <a class="" href="https://avocado-en.orderwebsite.com/gallery">
         <span class="pos-rlt inline">
          Gallery
         </span>
         <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
         </i>
        </a>
       </li>
       <li role="presentation">
        <a class="" href="https://avocado-en.orderwebsite.com/contact">
         <span class="pos-rlt inline">
          Contact
         </span>
         <i class="feather-icon icon-chevron-right pull-right m-t-3xxs">
         </i>
        </a>
       </li>
      </ul>
     </div>
    </aside>
    <div class="navigation-backdrop _toggle-aside-nav">
    </div>
   </div>
  </div>
  <div class="_page-content theme-content">
   <section class="theme-page-section section-height-custom background-fixed background-top" id="w592e4a3a-ce01-4e24-ae40-6db5fabc0285">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-2">
    </div>
    <div class="container _container-width">
     <div class="row row-flex flex-middle">
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h1>
         Healthy, colorful,
         <span>
          AVOCADO!
         </span>
        </h1>
       </div>
       <div class="page-widget text-widget">
        <h4>
         Enjoy our avocado specialties from Monday to Sunday!
        </h4>
       </div>
       <div class="page-widget button-widget flex-justify-left" id="w365a5d3a-5fae-4353-87e3-a0b7e578d626">
        <a class="btn button button-standard icon-holder bg-color-0 btn-size-large" href="https://avocado-en.orderwebsite.com/menu">
         <span "="" class="button-label inline text-ellipsis color-6" data-default-label="Button">
          Order online
         </span>
        </a>
       </div>
      </div>
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex flex-justify-center" id="w685890fd-30fa-4067-a462-67ba181411f6">
        <img alt="Image" class="page-widget-image image-size-auto" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/hero.png?v=0"/>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="theme-page-section section-height-custom background-fixed" id="w65613be3-922a-4555-afb3-dbc05494c656">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-0">
    </div>
    <div class="container _container-width">
     <div class="row row-flex column-reverse flex-middle">
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex flex-justify-center" id="w11a7750a-e862-4076-af52-b4ef54b01b43">
        <img alt="Image" class="page-widget-image" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/vegan2.jpg?v=0"/>
       </div>
      </div>
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h4>
         <span>
          Looking for something delicious and fit? You will love our restaurant!
         </span>
        </h4>
       </div>
       <div class="page-widget text-widget">
        <h2>
         <span>
          We invite you to AVOCADO! Here you will always eat something tasty
         </span>
        </h2>
       </div>
       <div class="page-widget text-widget">
        <p>
         <span>
          Are you ready for an explosion of flavor? At our restaurant, we combine our two passions: love for healthy living and delicious food. How is this possible? We opt for natural and fresh ingredients and unique spices. At its heart is the avocado!
         </span>
        </p>
       </div>
       <div class="page-widget spacer-widget spacer-small" id="w0131aa62-1601-4f88-ae66-f89a905060ca">
       </div>
       <div class="page-widget button-widget flex-justify-left" id="wfa5398e9-5e56-4e9d-8e1c-74d3f961ad1b">
        <a class="btn button button-outline icon-holder bg-color-6 btn-size-large" href="https://avocado-en.orderwebsite.com/menu">
         <span "="" class="button-label inline text-ellipsis color-6" data-default-label="Button">
          Order now
         </span>
        </a>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="theme-page-section section-height-custom" id="w1c75aa64-f60b-4e24-a7e1-2120965b8989">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-custom">
    </div>
    <div class="container _container-width">
     <div class="row row-flex flex-middle">
      <div class="col-sm-4 section-column" data-aos="fade-in">
       <div class="page-widget flex icon-box-widget widget-justify-center" id="w3cd8d462-f864-4ae9-b249-5ab0ab33bc70">
        <div class="single-icon-box icon-holder color-0 widget-vertical" data-icon="fa-coffee" data-icon-color="#004dff" data-icon-text="#000000">
         <i class="fa fa-coffee">
         </i>
         <div class="icon-box-text color-3">
          <h3 class="title">
           Aromatic coffee
          </h3>
          <p class="desc">
           We carefully select the coffee beans.
          </p>
         </div>
        </div>
       </div>
      </div>
      <div class="col-sm-4 section-column" data-aos="fade-in">
       <div class="page-widget flex icon-box-widget widget-justify-center" id="w6cdf9fea-399a-4314-89b4-d3e8a64a4313">
        <div class="single-icon-box icon-holder color-0 widget-vertical" data-icon="fa-bed" data-icon-color="#967d47" data-icon-text="#111111">
         <i class="fa fa-bed">
         </i>
         <div class="icon-box-text color-3">
          <h3 class="title">
           Comfort and chill
          </h3>
          <p class="desc">
           Our comfortable couches will make you forget about your troubles.
          </p>
         </div>
        </div>
       </div>
      </div>
      <div class="col-sm-4 section-column" data-aos="fade-in">
       <div class="page-widget flex icon-box-widget widget-justify-center" id="w0a127ce1-b5bb-440d-beb6-911c681767a9">
        <div class="single-icon-box icon-holder color-0 widget-vertical" data-icon="fa-cutlery" data-icon-color="#967d47" data-icon-text="#111111">
         <i class="fa fa-cutlery">
         </i>
         <div class="icon-box-text color-3">
          <h3 class="title">
           Healthy food
          </h3>
          <p class="desc">
           We cook healthily and tasty.
          </p>
         </div>
        </div>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="theme-page-section section-height-standard" id="w41cf9306-8182-490f-8e88-bf7851222cfe">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-2">
    </div>
    <div class="container _container-width">
     <div class="row row-flex flex-middle">
      <div class="col-sm-3 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h3 class="ql-align-center">
         <span>
          Healthy and delicious food
         </span>
        </h3>
       </div>
       <div class="page-widget text-widget">
        <p class="ql-align-center">
         We combine health with taste. We cook healthily, and all dishes on our menu are low in calories.
        </p>
       </div>
       <div class="page-widget spacer-widget spacer-custom" id="w2e535143-cb9b-4340-81ea-7c7e4ca2ab76">
       </div>
       <div class="page-widget text-widget">
        <h3 class="ql-align-center">
         <span>
          Sugar-free sweet desserts
         </span>
        </h3>
       </div>
       <div class="page-widget text-widget">
        <p class="ql-align-center">
         Is it possible to eat sweets while on a diet? Yes! All our desserts are healthy! Instead of sugar, we use the natural sweetness of fruits.
        </p>
       </div>
      </div>
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex" id="we541793f-2d61-416e-bd26-ecda9187233d">
        <img alt="Image" class="page-widget-image" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/vegan.jpg?v=0"/>
       </div>
      </div>
      <div class="col-sm-3 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h3 class="ql-align-center">
         <span>
          Aromatic coffee
         </span>
        </h3>
       </div>
       <div class="page-widget text-widget">
        <p class="ql-align-center">
         Coffee from freshly roasted and carefully selected beans tastes delicious every day. Come and try it!
        </p>
       </div>
       <div class="page-widget spacer-widget spacer-custom" id="we92b3805-9d7d-41e9-9fb7-327e00dacab0">
       </div>
       <div class="page-widget text-widget">
        <h3 class="ql-align-center">
         <span>
          The power of freshness
         </span>
        </h3>
       </div>
       <div class="page-widget text-widget">
        <p class="ql-align-center">
         We carefully select our ingredients. We only order fresh and quality products from local suppliers, including vegetables and fruits.
        </p>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="theme-page-section section-height-custom" id="wb558928a-80de-4def-a26e-c1ad8d3b1455">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-1">
    </div>
    <div class="container _container-width">
     <div class="row row-flex flex-middle">
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h4>
         <span>
          Authentic fit food
         </span>
        </h4>
       </div>
       <div class="page-widget text-widget">
        <h2>
         <span>
          Avocado is our superhero!
         </span>
        </h2>
       </div>
       <div class="page-widget text-widget">
        <p>
         <span>
          We focus on health and taste. In the heart of our kitchen, we put avocado, the healthiest fruit in the world, which is a real mine of vitamins and nutrients! All our dishes are created based on this beauty.
         </span>
        </p>
       </div>
       <div class="page-widget spacer-widget spacer-custom" id="wa8b75aab-cde8-4374-8a54-b9016c76a457">
       </div>
       <div class="page-widget button-widget flex-justify-left" id="w8cc831ed-eccc-48b6-98d0-35633d887fdd">
        <a class="btn button button-outline icon-holder bg-color-0 btn-size-large" href="https://avocado-en.orderwebsite.com/menu">
         <span "="" class="button-label inline text-ellipsis color-0" data-default-label="Button">
          Order now
         </span>
        </a>
       </div>
      </div>
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex flex-justify-center" id="w015001d6-3f1b-4f0b-a58d-d78dc59f80c8">
        <img alt="Image" class="page-widget-image" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/person.jpg?v=0"/>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="theme-page-section section-height-standard" id="wf3004da6-2db2-47dc-b52b-867e00256ad1">
    <div class="section-video-layer" data-vbg="">
    </div>
    <div class="section-overlay color-custom">
    </div>
    <div class="container-fluid _container-width">
     <div class="row row-flex flex-middle">
      <div class="col-sm-3 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex flex-justify-center" id="w355e1c64-ec30-42cb-8b8c-96444ba9b23c">
        <img alt="Image" class="page-widget-image image-size-auto" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/vegan_AFouckhG.jpg?v=0"/>
       </div>
      </div>
      <div class="col-sm-6 section-column" data-aos="fade-in">
       <div class="page-widget text-widget">
        <h2 class="ql-align-center">
         How about healthy meal delivery service?
        </h2>
       </div>
       <div class="page-widget text-widget">
        <h4 class="ql-align-center">
         You can order a catering service from us. What does this mean? Why is it worth it? Every day, we will deliver your meals according to your diet and culinary preferences. Contact us for more details.
        </h4>
       </div>
       <div class="page-widget spacer-widget spacer-custom" id="w869064ed-91e3-4b50-9987-31f3e704374c">
       </div>
       <div class="page-widget button-widget flex-justify-center" id="w7d1bbc02-37d7-4a68-9f24-6474ce9014c7">
        <a class="btn button button-standard icon-holder bg-color-0 btn-size-large" href="https://avocado-en.orderwebsite.com/menu">
         <span "="" class="button-label inline text-ellipsis color-6" data-default-label="Button">
          Order online
         </span>
        </a>
       </div>
      </div>
      <div class="col-sm-3 section-column" data-aos="fade-in">
       <div class="page-widget image-widget flex flex-justify-center" id="w8a528f68-ec48-4617-b03f-d2b654b25c87">
        <img alt="Image" class="page-widget-image image-size-auto" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/vegan3.jpg?v=0"/>
       </div>
      </div>
     </div>
    </div>
   </section>
  </div>
  <div class="page-footer">
   <footer class="theme-footer">
    <div class="container theme-footer-inner">
     <div class="row">
      <div class="col-sm-12 text-center">
       <a class="brand footer-logo-standard" href="index.html">
        <img alt="Avocado" class="img-responsive inline m-b-md" height="36" src="https://cs.cdn-upm.com/themes/98dfb947-4a04-11ed-8bca-525400080621/assets-6/logo.png?v=0" width="160">
        </img>
       </a>
       <div class="footer-socials">
       </div>
      </div>
     </div>
     <div class="row">
      <div class="col-sm-12 m-t-lg m-b-lg">
       <ul class="horizontal-list text-center m-b-none">
        <li>
         <a class="active" href="index.html">
          Home
         </a>
        </li>
        <li>
         <a class="" href="https://avocado-en.orderwebsite.com/about-us">
          About us
         </a>
        </li>
        <li>
         <a class="" href="https://avocado-en.orderwebsite.com/gallery">
          Gallery
         </a>
        </li>
        <li>
         <a class="" href="https://avocado-en.orderwebsite.com/contact">
          Contact
         </a>
        </li>
        <li>
         <a class="" href="https://avocado-en.orderwebsite.com/menu">
          Order online
         </a>
        </li>
        <li>
         <a class="_show-unsubscribe-modal" href="https://avocado-en.orderwebsite.com/api/v2/newsletterUnsubscribe/98dfb947-4a04-11ed-8bca-525400080621">
          Unsubscribe
         </a>
        </li>
       </ul>
      </div>
     </div>
     <div class="row">
      <div class="col-sm-12 m-b-lg">
       <ul class="address-list horizontal-list text-center">
        <li>
         <svg fill="none" height="25" stroke="currentColor" stroke-width="2" viewbox="0 0 25 25" width="25" xmlns="http://www.w3.org/2000/svg">
          <path d="M15.5499 5.15723C16.5267 5.34779 17.4243 5.82548 18.128 6.52916C18.8317 7.23284 19.3094 8.13049 19.4999 9.10723M15.5499 1.15723C17.5792 1.38266 19.4715 2.2914 20.9162 3.73423C22.3608 5.17707 23.2719 7.06824 23.4999 9.09723M22.4999 17.0772V20.0772C22.5011 20.3557 22.444 20.6314 22.3324 20.8866C22.2209 21.1418 22.0572 21.3708 21.852 21.5591C21.6468 21.7474 21.4045 21.8907 21.1407 21.9799C20.8769 22.0692 20.5973 22.1023 20.3199 22.0772C17.2428 21.7429 14.2869 20.6914 11.6899 19.0072C9.27376 17.4719 7.22527 15.4234 5.68993 13.0072C3.99991 10.3984 2.94818 7.42822 2.61993 4.33723C2.59494 4.06069 2.62781 3.78199 2.71643 3.51885C2.80506 3.25572 2.9475 3.01392 3.1347 2.80885C3.32189 2.60378 3.54974 2.43994 3.80372 2.32775C4.05771 2.21556 4.33227 2.15749 4.60993 2.15723H7.60993C8.09524 2.15245 8.56572 2.32431 8.93369 2.64076C9.30166 2.95721 9.54201 3.39667 9.60993 3.87723C9.73656 4.83729 9.97138 5.77995 10.3099 6.68723C10.4445 7.04515 10.4736 7.43414 10.3938 7.80811C10.3141 8.18207 10.1288 8.52534 9.85993 8.79723L8.58993 10.0672C10.0135 12.5708 12.0864 14.6437 14.5899 16.0672L15.8599 14.7972C16.1318 14.5284 16.4751 14.3431 16.8491 14.2633C17.223 14.1836 17.612 14.2127 17.9699 14.3472C18.8772 14.6858 19.8199 14.9206 20.7799 15.0472C21.2657 15.1158 21.7093 15.3604 22.0265 15.7347C22.3436 16.109 22.5121 16.5868 22.4999 17.0772Z" stroke-linecap="round" stroke-linejoin="round">
          </path>
         </svg>
         <a href="tel:+1 123123123">
          +1 123123123
         </a>
        </li>
        <li>
         <svg fill="none" height="25" stroke="currentColor" stroke-width="2" viewbox="0 0 24 25" width="24" xmlns="http://www.w3.org/2000/svg">
          <path d="M4 4.15723H20C21.1 4.15723 22 5.05723 22 6.15723V18.1572C22 19.2572 21.1 20.1572 20 20.1572H4C2.9 20.1572 2 19.2572 2 18.1572V6.15723C2 5.05723 2.9 4.15723 4 4.15723Z" stroke-linecap="round" stroke-linejoin="round">
          </path>
          <path d="M22 6.15723L12 13.1572L2 6.15723" stroke-linecap="round" stroke-linejoin="round">
          </path>
         </svg>
         <a href="mailto:contact@upmenu.com" target="_blank">
          contact@upmenu.com
         </a>
        </li>
       </ul>
      </div>
     </div>
     <div class="row">
      <div class="col-sm-12 text-center">
       <p class="theme-footer-powered-by text-center m-t-none">
        Online Food Ordering System by
        <a href="https://www.upmenu.com" target="_blank">
         <svg alt="UpMenu" class="_powered-by-logo" viewbox="0 0 240.2 54.5" x="0px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" y="0px">
          <path d="M18.5,19c-0.6,0-1.1,0.5-1.1,1.1v2.5c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1v-2.5C19.5,19.5,19.1,19,18.5,19z">
          </path>
          <path d="M18.5,51.5c-0.6,0-1.1,0.5-1.1,1.1v0.8c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1v-0.8 C19.5,52,19.1,51.5,18.5,51.5z">
          </path>
          <path d="M27.2,46.6c-0.6,0-1.1,0.5-1.1,1.1v3.7c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1v-3.7C28.3,47.1,27.8,46.6,27.2,46.6z">
          </path>
          <path d="M36.9,23.2V9.7c0-1.6-1-3-2.4-3.6L19.9,0.3C19.4,0.1,19,0,18.5,0s-1,0.1-1.4,0.3L2.4,6.1C0.9,6.7,0,8.1,0,9.7
                        v13.5v12v5.6l5.2-2.1l13.2-5.2l13.2,5.2l5.2,2.1v-5.6L36.9,23.2L36.9,23.2z M19.5,29.9v-3.4c0-0.6-0.5-1.1-1.1-1.1
                        s-1.1,0.5-1.1,1.1v3.4L3.7,35.3v-12V9.8l13.6-5.4V16c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1V4.3l13.6,5.4v13.5v12L19.5,29.9z">
          </path>
          <path d="M26.1,42.6c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1v-2.2l-2.2-0.9V42.6z">
          </path>
          <path d="M17.4,36.9v10.7c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1V36.9l-1.1-0.4L17.4,36.9z">
          </path>
          <path d="M8.6,50.2c0,0.6,0.5,1.1,1.1,1.1s1.1-0.5,1.1-1.1V39.5l-2.2,0.9L8.6,50.2z">
          </path>
          <path d="M70,42.7c-0.2,0.2-0.5,0.4-0.8,0.4h-2.8c-0.3,0-0.6-0.1-0.8-0.4c-0.2-0.2-0.3-0.5-0.3-0.8v-1.1c-1.6,0.9-3.1,1.6-4.4,2
						c-1.4,0.5-2.9,0.7-4.6,0.7c-3.4,0-5.9-1.1-7.5-3.2s-2.4-5.1-2.4-9V12.1c0-0.3,0.1-0.6,0.3-0.8s0.5-0.4,0.8-0.4h2.7
						c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8v19.2c0,1.5,0.1,2.7,0.4,3.7c0.2,1,0.6,1.8,1.1,2.4s1.2,1,1.9,1.3
						c0.8,0.3,1.7,0.4,2.8,0.4c1.2,0,2.5-0.2,3.7-0.6c1.3-0.4,2.6-1,3.9-1.6V12.3c0-0.3,0.1-0.6,0.3-0.8s0.5-0.4,0.8-0.4H69
						c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8v29.8C70.3,42.2,70.2,42.5,70,42.7z">
          </path>
          <path d="M102,29.2c0,4.6-1,8.1-3.1,10.6c-2,2.5-5.1,3.7-9.3,3.7c-0.9,0-1.9-0.1-3.1-0.2S84.1,43,83,42.8v10.6
						c0,0.3-0.1,0.6-0.3,0.8c-0.2,0.2-0.5,0.3-0.8,0.3H79c-0.3,0-0.5-0.1-0.7-0.3S78,53.7,78,53.4V13.2c0-0.4,0.1-0.7,0.2-0.9
						s0.5-0.4,1-0.5c1.5-0.4,3.2-0.7,5-1c1.9-0.3,3.6-0.4,5.4-0.4c2.1,0,4,0.3,5.5,1s2.8,1.7,3.9,2.9c1,1.3,1.8,2.8,2.3,4.6
						s0.7,3.7,0.7,5.9L102,29.2L102,29.2z M97,25c0-1.5-0.1-2.9-0.4-4.1c-0.2-1.2-0.7-2.3-1.2-3.2c-0.6-0.9-1.3-1.6-2.3-2
						C92.2,15.2,91,15,89.6,15c-1.2,0-2.4,0.1-3.6,0.2c-1.2,0.1-2.2,0.3-3,0.4v23.1c0.9,0.2,2.1,0.3,3.3,0.4c1.3,0.1,2.4,0.1,3.3,0.1
						c1.4,0,2.6-0.2,3.5-0.7c0.9-0.4,1.7-1.1,2.3-2c0.6-0.9,1-1.9,1.2-3.2c0.2-1.2,0.4-2.6,0.4-4.2L97,25L97,25z">
          </path>
          <path d="M145.9,42.7c-0.2,0.2-0.5,0.4-0.8,0.4h-2.7c-0.3,0-0.6-0.1-0.8-0.4c-0.2-0.2-0.3-0.5-0.3-0.8V21.2c0-1.2-0.1-2.2-0.4-3
						s-0.6-1.5-1.1-1.9c-0.5-0.5-1-0.8-1.6-1s-1.3-0.3-2-0.3c-1,0-2.1,0.2-3.3,0.5c-1.2,0.3-2.3,0.7-3.4,1.1c0.3,1.3,0.5,2.8,0.5,4.6V42
						c0,0.3-0.1,0.6-0.3,0.8s-0.4,0.4-0.7,0.4h-3c-0.3,0-0.5-0.1-0.7-0.4c-0.2-0.2-0.3-0.5-0.3-0.8V21.2c0-1.3-0.1-2.3-0.3-3.1
						c-0.2-0.8-0.4-1.4-0.8-1.9c-0.4-0.4-0.8-0.7-1.4-0.9c-0.5-0.2-1.2-0.2-1.9-0.2c-1,0-2.1,0.2-3.3,0.7c-1.2,0.4-2.4,1-3.6,1.6V42
						c0,0.3-0.1,0.6-0.3,0.8c-0.2,0.2-0.4,0.4-0.7,0.4h-3c-0.3,0-0.5-0.1-0.7-0.4c-0.2-0.2-0.3-0.5-0.3-0.8V12.2c0-0.3,0.1-0.6,0.3-0.8
						c0.2-0.2,0.5-0.4,0.8-0.4h2.8c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8v1.1c1.1-0.7,2.4-1.4,3.8-1.9s2.9-0.8,4.5-0.8
						c1.2,0,2.3,0.2,3.2,0.6c0.9,0.4,1.7,1,2.5,1.8c1.6-0.8,3.2-1.4,4.8-1.8c1.6-0.4,3.1-0.6,4.6-0.6c1.3,0,2.5,0.2,3.6,0.5
						s2.1,0.9,2.9,1.8c0.8,0.8,1.5,1.9,2,3.3s0.7,3.1,0.7,5.1V42C146.3,42.2,146.2,42.5,145.9,42.7z">
          </path>
          <path d="M178,27.4c0,0.9-0.4,1.3-1.1,1.3h-19v0.8c0,3.5,0.7,5.9,2,7.4c1.3,1.4,3.3,2.2,5.9,2.2c1.7,0,3.4-0.1,5.3-0.2
						s3.5-0.3,4.8-0.5h0.3c0.2,0,0.4,0.1,0.6,0.2s0.3,0.3,0.3,0.6v2c0,0.4-0.1,0.7-0.2,0.9c-0.2,0.2-0.5,0.4-0.9,0.4
						c-1.6,0.3-3.2,0.6-4.8,0.7c-1.6,0.2-3.4,0.3-5.3,0.3c-1.6,0-3.2-0.2-4.8-0.7c-1.5-0.4-2.9-1.2-4.1-2.3c-1.2-1.1-2.2-2.5-2.9-4.3
						s-1.1-4-1.1-6.8v-5.3c0-4.2,1.1-7.5,3.2-10s5.3-3.7,9.3-3.7c2.1,0,3.9,0.3,5.4,1c1.5,0.6,2.8,1.6,3.9,2.7c1,1.2,1.8,2.6,2.3,4.3
						s0.8,3.6,0.8,5.6v3.4C177.9,27.4,178,27.4,178,27.4z M173,24.2c0-3.3-0.6-5.7-1.8-7.1c-1.2-1.4-3.1-2.1-5.6-2.1s-4.3,0.7-5.6,2.2
						s-2,3.8-2,6.9v0.5h15L173,24.2L173,24.2z">
          </path>
          <path d="M208.4,42.7c-0.2,0.2-0.5,0.4-0.8,0.4h-2.7c-0.3,0-0.6-0.1-0.8-0.4c-0.2-0.2-0.3-0.5-0.3-0.8V22.7c0-1.5-0.1-2.7-0.4-3.7
						c-0.2-1-0.6-1.8-1.1-2.4s-1.2-1-2-1.3s-1.7-0.4-2.8-0.4c-1.2,0-2.5,0.2-3.7,0.6c-1.3,0.4-2.6,1-3.9,1.6v24.6c0,0.3-0.1,0.6-0.3,0.8
						s-0.5,0.4-0.8,0.4h-2.9c-0.3,0-0.6-0.1-0.8-0.4c-0.2-0.2-0.3-0.5-0.3-0.8V11.9c0-0.3,0.1-0.6,0.3-0.8s0.5-0.4,0.8-0.4h2.8
						c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8V13c1.6-0.9,3.1-1.6,4.4-2c1.4-0.5,2.9-0.7,4.6-0.7c3.4,0,5.9,1,7.5,3.2
						c1.6,2.1,2.4,5.1,2.4,9v19.2C208.7,42.2,208.6,42.5,208.4,42.7z">
          </path>
          <path d="M240,42.7c-0.2,0.2-0.5,0.4-0.8,0.4h-2.8c-0.3,0-0.6-0.1-0.8-0.4c-0.2-0.2-0.3-0.5-0.3-0.8v-1.1c-1.6,0.9-3.1,1.6-4.4,2
						c-1.4,0.5-2.9,0.7-4.6,0.7c-3.4,0-5.9-1.1-7.5-3.2s-2.4-5.1-2.4-9V12.1c0-0.3,0.1-0.6,0.3-0.8s0.5-0.4,0.8-0.4h2.7
						c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8v19.2c0,1.5,0.1,2.7,0.4,3.7c0.2,1,0.6,1.8,1.1,2.4s1.2,1,1.9,1.3
						c0.8,0.3,1.7,0.4,2.8,0.4c1.2,0,2.5-0.2,3.7-0.6c1.3-0.4,2.6-1,3.9-1.6V12.3c0-0.3,0.1-0.6,0.3-0.8s0.5-0.4,0.8-0.4h2.8
						c0.3,0,0.6,0.1,0.8,0.4c0.2,0.2,0.3,0.5,0.3,0.8v29.8C240.3,42.2,240.2,42.5,240,42.7z">
          </path>
         </svg>
        </a>
       </p>
      </div>
     </div>
    </div>
   </footer>
  </div>
  <!-- COOKIE IN IFRAME BUGFIX // -->
  <!-- http://stackoverflow.com/questions/10060820/does-the-technique-for-setting-third-party-cookies-in-iframes-in-safari-still-wo -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_cookie-iniframe-bugfix-modal">
   <div class="modal-dialog modal-lg">
    <div class="modal-content">
     <div class="modal-header">
      <h4 class="modal-title">
       Problem with cookies.
      </h4>
     </div>
     <div class="modal-body" id="_cookie-iniframe-bugfix-modal-body">
      It looks like cookies on your browser are disabled or they do not operate properly.
     </div>
    </div>
   </div>
  </div>
  <!-- // COOKIE BUGFIX -->
  <!-- NEWSLETTER SUBSCRIPTION MODAL (FOR AUTOMATIC) // -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_newsletter-subscription-modal">
   <div class="modal-dialog modal-lg">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content" id="_newsletter-subscription-modal-content">
    </div>
   </div>
  </div>
  <!-- // NEWSLETTER SUBSCRIPTION MODAL (FOR AUTOMATIC) -->
  <!-- GET MY POSITION MODAL // -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_show-modal-geolocation">
   <div class="modal-dialog modal-lg">
    <div class="modal-header">
     <h5 class="modal-title">
      Move the map to the place of delivery
     </h5>
     <div class="modal-close">
      <button class="pull-right _show-modal-geolocation-close" type="button">
       <img src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
      </button>
     </div>
    </div>
    <div class="modal-content">
     <div class="modal-body" id="_show-modal-geolocation-body">
      <div id="getMyLocation">
      </div>
     </div>
     <div class="modal-footer">
      <div class="address pull-left" id="_show-modal-geolocation-address">
       <div class="first-address">
        <span class="street">
        </span>
       </div>
       <div class="second-address">
        <span class="postal-code">
        </span>
        <span class="city">
        </span>
        <span class="region">
        </span>
       </div>
      </div>
      <button class="btn btn-upmenu _show-modal-geolocation-btn" type="button">
       Confirm the location
      </button>
     </div>
    </div>
   </div>
  </div>
  <!-- // GET MY POSITION MODAL -->
  <!-- GENERIC MODALS -->
  <div class="modal fade" data-backdrop="static" id="_cms-modal-big">
   <div class="modal-dialog modal-lg">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content" id="_cms-modal-big-content">
    </div>
   </div>
  </div>
  <div class="modal fade cms-modal modal-narrow" data-backdrop="static" id="_cms-modal-narrow">
   <div class="modal-dialog">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content" id="_cms-modal-narrow-content">
    </div>
   </div>
  </div>
  <div class="modal fade cms-modal order-form-fullscreen" data-backdrop="static" id="_cms-modal">
   <div class="modal-dialog">
    <div class="modal-content" id="_cms-modal-content">
    </div>
   </div>
  </div>
  <!-- /GENERIC MODAL -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_cms-modal-adults">
   <div class="modal-dialog">
    <div class="modal-content" id="_cms-modal-adults-content">
     <div class="modal-body">
      <div class="row">
       <div class="col-sm-12 text-center modal-logo">
        <img alt="Hold image" src="https://static.cdn-upm.com/media/1202/images/themes/stop-hand.svg"/>
       </div>
      </div>
      <div class="row">
       <div class="col-sm-12">
        <p class="text-center m-b-lg">
         Some of the items in our menu are available only for adults.
         <br/>
         <strong>
          Are you an adult person?
         </strong>
        </p>
       </div>
      </div>
      <div class="row">
       <div class="col-xs-12 col-sm-6">
        <button class="btn btn-upmenu btn-block btn-lg _adult-items-allow" type="button">
         Yes
        </button>
       </div>
       <div class="col-xs-12 col-sm-6">
        <button aria-label="Close" class="btn btn-default btn-block btn-lg _adult-items-deny" data-dismiss="modal" type="button">
         No
        </button>
       </div>
      </div>
     </div>
    </div>
   </div>
  </div>
  <!-- /GENERIC MODAL -->
  <div class="modal cms4-modal fade" id="form-gdpr">
   <div class="modal-dialog modal-md">
    <div class="modal-content">
     <div class="modal-header">
      <h4 class="modal-title">
       <button aria-hidden="true" class="pull-right" data-dismiss="modal" type="button">
        <i class="feather-icon icon-x">
        </i>
       </button>
       Privacy policy
      </h4>
     </div>
     <div class="modal-body" id="form-gdpr-content">
     </div>
    </div>
   </div>
  </div>
  <div class="modal fade cms-modal" data-backdrop="static" id="form-adult-terms-modal">
   <div class="modal-dialog">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content">
     <div class="modal-body" id="form-adult-terms-modal-content">
     </div>
    </div>
   </div>
  </div>
  <!-- TERMS OF SERVICE MODAL // -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_terms-of-service-modal">
   <div class="modal-dialog modal-lg">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img alt="Close" src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content">
     <div class="modal-body" id="_terms-of-service-modal-body">
     </div>
    </div>
   </div>
  </div>
  <!-- // TERMS OF SERVICE MODAL -->
  <!-- PRIVACY POLICY MODAL // -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_privacy-policy-modal">
   <div class="modal-dialog modal-lg">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img alt="Close" src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content">
     <div class="modal-body" id="_privacy-policy-modal-body">
     </div>
    </div>
   </div>
  </div>
  <!-- // PRIVACY POLICY MODAL -->
  <!-- UNSUBSCRIBE MODAL // -->
  <div class="modal fade cms-modal" data-backdrop="static" id="_unsubscribe-modal">
   <div class="modal-dialog">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img alt="Close" src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content">
     <div class="modal-body" id="_unsubscribe-modal-body">
     </div>
    </div>
   </div>
  </div>
  <!-- // UNSUBSCRIBE MODAL -->
  <!-- POPUPS // -->
  <!-- POPUPS // -->
  <!-- CMS4 POPUPS -->
  <div class="modal fade cms4-modal" data-backdrop="static" id="cms4-popups">
   <div class="modal-dialog">
    <div class="modal-close">
     <button aria-label="Close" class="pull-right" data-dismiss="modal" type="button">
      <img alt="Close" src="https://static.cdn-upm.com/media/1202/images/icons/x-white.svg"/>
     </button>
    </div>
    <div class="modal-content">
     <div class="modal-body no-padding" id="cms4-popups-content">
     </div>
    </div>
   </div>
  </div>
  <!-- EDIT MODE // -->
  <!-- CSS LIBS -->
  <link href="https://static.cdn-upm.com/media/1202/bottomAllCms4.css" rel="stylesheet" type="text/css">
   <!-- // CSS LIBS -->
   <!-- JS LIBS -->
   <script async="" defer="" src="https://appleid.cdn-apple.com/appleauth/static/jsapi/appleid/1/en_US/appleid.auth.js" type="text/javascript">
   </script>
   <script src="https://static.cdn-upm.com/media/1202/allCms4.js" type="text/javascript">
   </script>
   <!-- // JS LIBS  -->
   <!-- EDIT MODE // -->
   <script src="https://static.cdn-upm.com/media/1202/api3/js/cms-api-utils.js" type="text/javascript">
   </script>
   <script src="https://static.cdn-upm.com/media/1202/js/utils.js" type="text/javascript">
   </script>
   <script src="https://static.cdn-upm.com/media/1202/api3/js/edit_v2/google-webfonts.js" type="text/javascript">
   </script>
   <script src="https://static.cdn-upm.com/media/1202/api3.js" type="text/javascript">
   </script>
   <!-- EDIT MODE // -->
   <script src="https://static.cdn-upm.com/media/1202/api3/js/edit_v2/cms-api-utils.js" type="text/javascript">
   </script>
   <script src="assets/js/script_extracted.js">
   </script>
  </link>
 </body>
</html>
